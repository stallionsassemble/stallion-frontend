...
export function RecentWins() {
  ...
  // We only need one copy of scrollingWinsBase here, because the HorizontalMarquee
  // will responsibly duplicate it for the seamless loop.
  // Using one copy reduces DOM node count initially.
  const marqueeData = scrollingWinsBase
  ...
  const renderCard = (
    ...
    return (
      ...
      >
        {/* Price Badge */}
        <div
          className={`flex absolute -top-3 -left-3 px-3 py-1 rounded-t-md rounded-br-md ${badgeBgClass} text-white text-[10px] font-normal font-inter leading-3.5`}
        >
          {isHighlighted && (
            <Image
          ...
          {isHighlighted ? win.amount.slice(1) : win.amount}
        </div>

        <div className='flex items-center gap-3 mt-2'>
          {/* Winner Avatar */}
          <div
            className='w-[43.04px] h-[43.04px] rounded-full'
            style={{ boxShadow: '0px 0px 0px 1.79px #34D39980' }}
          >
            <img
              className='w-[43.04px] h-[43.04px] object-cover rounded-full'
              alt=''
              src={'https://placehold.co/43x43'}
              loading='lazy'
            />
          </div>
          ...
          <div className='flex flex-col ml-1'>
            <div
              className={`${winnerClass} font-normal text-[12px] leading-[17.5px] font-inter`}
            >
      ...
      <div className='hidden md:flex flex-col gap-10 container mx-auto max-w-6xl px-4'>
        ...
        <div className='flex items-center gap-[29px] w-full'>
          ...
          <HorizontalMarquee
            direction='left'
            speed={40}
            className='flex-1 mask-gradient'
          >
        ...
        <div className='flex items-center gap-[29px] w-full'>
          ...
          <HorizontalMarquee
            ...
            className='flex-1 mask-gradient'
          >
            {row2Marquee.map((win, index) =>
              renderCard(win, index + 100, false)
            )}
          </HorizontalMarquee>
        </div>
        ...
        <div className='flex items-center gap-[29px] w-full'>
          ...
          <HorizontalMarquee
            ...
            className='flex-1 mask-gradient'
          >
            {row3Marquee.map((win, index) =>
              renderCard(win, index + 200, false)
            )}
          </HorizontalMarquee>
        </div>
      </div>
